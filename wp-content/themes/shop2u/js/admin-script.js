( function( $ ){
    $( document ).ready( function(){
      $( '.shop2u-btn-get-started' ).on( 'click', function( e ) {
          e.preventDefault();
          $( this ).html( 'Processing.. Please wait' ).addClass( 'updating-message' );
          $.post( shop2u_ajax_object.ajax_url, { 'action' : 'install_act_plugin' }, function( response ){
        ...
        $.ajax( ajaxurl,
          {
            type: 'POST',
            data: {
              action: 'shop2u_dismissed_notice_handler',
              type: type,
            }
          } );
      } );
