<?php
class Shop2u_Customizer_Notify_Section extends WP_Customize_Section {
	
	public $type = 'shop2u-customizer-notify-section';
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'shop2u_cust_notify_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'shop2u_cust_notify_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		...
		$shop2u_customizer_notify_show_recommended_actions = isset($option['shop2u_customizer_notify_show'])?$option['shop2u_customizer_notify_show']:'';
		foreach ( $shop2u_customizer_notify_recommended_actions as $key => $shop2u_customizer_notify_recommended_action ) {
			if ( $shop2u_customizer_notify_show_recommended_actions[ $shop2u_customizer_notify_recommended_action['id'] ] === false ) {
				continue;
			}
			if ( $shop2u_customizer_notify_recommended_action['check'] ) {
				continue;
			}

			$shop2u_customizer_notify_recommended_action['index'] = $key + 1;

			if ( isset( $shop2u_customizer_notify_recommended_action['plugin_slug'] ) ) {
				$active = $this->check_active( $shop2u_customizer_notify_recommended_action['plugin_slug'] );
				$shop2u_customizer_notify_recommended_action['url'] = $this->create_action_link( $active['needs'], $shop2u_customizer_notify_recommended_action['plugin_slug'] );
				if ( $active['needs'] !== 'install' && $active['status'] ) {
					$shop2u_customizer_notify_recommended_action['class'] = 'active';
				} else {
					$shop2u_customizer_notify_recommended_action['class'] = '';
				}

				switch ( $active['needs'] ) {
					case 'install':
						$shop2u_customizer_notify_recommended_action['button_class'] = 'install-now button';
						$shop2u_customizer_notify_recommended_action['button_label'] = $install_button_label;
						break;
					case 'activate':
						$shop2u_customizer_notify_recommended_action['button_class'] = 'activate-now button button-primary';
						$shop2u_customizer_notify_recommended_action['button_label'] = $activate_button_label;
						break;
					case 'deactivate':
						$shop2u_customizer_notify_recommended_action['button_class'] = 'deactivate-now button';
						$shop2u_customizer_notify_recommended_action['button_label'] = $deactivate_button_label;
						break;
				}
			}
			$formatted_array[] = $shop2u_customizer_notify_recommended_action;
		}// End foreach().

		$customize_plugins = array();

		$shop2u_customizer_notify_show_recommended_plugins = isset($option['shop2u_customizer_notify_show_recommended_plugins'])?$option['shop2u_customizer_notify_show_recommended_plugins']:'';

		foreach ( $shop2u_customizer_notify_recommended_plugins as $slug => $plugin_opt ) {

			if ( ! $plugin_opt['recommended'] ) {
				continue;
			}

			if ( isset( $shop2u_customizer_notify_show_recommended_plugins[ $slug ] ) && $shop2u_customizer_notify_show_recommended_plugins[ $slug ] ) {
				continue;
			}
		...
		$json['total_actions']       = count( $shop2u_customizer_notify_recommended_actions );
		...
		<# if( data.recommended_actions.length > 0 || data.recommended_plugins.length > 0 ){ #>
			<li id="accordion-section-{{ data.id }}" class="accordion-section control-section control-section-{{ data.type }} cannot-expand">
				...
				<h3 class="accordion-section-title">
					...
					<# if( data.recommended_actions.length > 0 ){ #>
						<span class="shop2u-customizer-plugin-notify-actions-count">
							<span class="current-index">{{ data.recommended_actions[0].index }}</span>
							{{ data.total_actions }}
						</span>
					<# } #>
				</h3>
				<div class="shop2u-theme-recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_actions.length > 0 ){ #>
						<# for (action in data.recommended_actions) { #>
							<div class="shop2u-recommeded-actions-container epsilon-required-actions" data-index="{{ data.recommended_actions[action].index }}">
								<# if( !data.recommended_actions[action].check ){ #>
									<div class="shop2u-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_actions[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no shop2u-customizer-notify-dismiss-recommended-action" id="{{ data.recommended_actions[action].id }}"></span>
					...
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="shop2u-recommeded-actions-container epsilon-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="shop2u-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_plugins[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no shop2u-customizer-notify-dismiss-button-recommended-plugin" id="{{ data.recommended_plugins[action].id }}"></span>
